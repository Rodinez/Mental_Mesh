...
func _process(delta):
	...
	if Input.is_action_pressed("move_right") and Global.player_now == 1 and Global.can_move:
		velocity.x += 1
		$AnimatedSprite2D.play("direita")
	if Input.is_action_pressed("move_left") and Global.player_now == 1 and Global.can_move:
		velocity.x -= 1   
		$AnimatedSprite2D.play("esquerda")
	if Input.is_action_pressed("move_down") and Global.player_now == 1 and Global.can_move:
		velocity.y += 1  
		$AnimatedSprite2D.play("frente")
	if Input.is_action_pressed("move_up") and Global.player_now == 1 and Global.can_move:
		velocity.y -= 1
		$AnimatedSprite2D.play("costa")
	if Input.is_action_just_released("change") and Global.player_now == 1 and Global.can_move:
